...
flags.DEFINE_string('log1_dir', 'logs', 'Where to save training models')
...
def prepare_dirs(config, prefix=['HMR']):
    ...
    if config.load_path:
        ...
        config.model_dir = osp.join(config.log1_dir, save_name)

    for path in [config.log1_dir, config.model_dir]:
